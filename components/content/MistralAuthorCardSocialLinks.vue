<template>
    <div class="flex space-x-4">
        <NuxtLink
            v-for="(url, platform) in filteredSocials"
            :key="platform"
            :to="url"
        ...
        >
            ...
            <svg
                ...
                :viewBox="icons[platform]?.viewBox"
...
const filteredSocials = computed(() => {
    return Object.entries(props.socials)
        .filter(([key, value]) => {
            if (!icons[key] && value) {
                console.warn(`Social platform "${key}" is not supported. Available platforms: ${Object.keys(icons).join(', ')}`)
                return false
            }
            return value
        })
